...
const createWindow = () => {
	...
	ipcMain.on('FULLSCREEN_TOGGLE', (_, isFullscreen) => {
		if (isFullscreen) {
			originalBounds.width = win.getSize()[0];
			originalBounds.height = win.getSize()[1];
			win.fullScreen = true;
		} else {
			win.fullScreen = false;
			win.setSize(originalBounds.width, originalBounds.height, true);
		}
	});
